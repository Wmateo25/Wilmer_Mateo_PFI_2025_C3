...
/* Parte 2 contendrá operaciones con complejos y su menu */
void menu_complejos(void);

/* Parte 3 contendrá operaciones con matrices y su menu */
...
double complex div_complex(double complex a, double complex b) {
    if (cabs(b) < 1e-15) {
        fprintf(stderr, "Error: division por cero (complejo).\n");
        return NAN + NAN*I;
    }
    return a / b;
}
...
void leer_dimensiones(int *r, int *c)
 {
    while (1) {
        ...
        printf("Dimensiones invalidas. Deben estar entre 1 y %d.\n", MAX_N);
    }
}
...
/* division de matrices: A * inv(B) cuando es posible (B cuadrada e invertible y dimensiones compatibles) */
